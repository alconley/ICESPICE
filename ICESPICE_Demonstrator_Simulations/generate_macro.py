...
def generate_all_electrons_macro(
    ...
    File = f"run_{runno}_PIPS{detector_um}_f{f_mm}mm_g{g_mm}mm_n{N_total}_207BiThickness{thickness_nm}nm_Cut{cut}um_AllElectrons_theta{theta_deg}_phi{phi_deg}"

    def file_for(label):
        return f"run_{runno}_PIPS{detector_um}_f{f_mm}mm_g{g_mm}mm_n{N_total}_207BiThickness{thickness_nm}nm_Cut{cut}um_{label}_theta{theta_deg}_phi{phi_deg}"
